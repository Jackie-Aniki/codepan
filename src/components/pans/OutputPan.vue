...
export default {
  ...
  methods: {
    ...
    async run() {
      ...
      const transformed = { html: '', code: '', css: '' }
      const scripts = []
      ...
      try {
        ...
        localStorage.setItem('codepan.js', this.code.code)
      } catch (err) {
        this.setIframeStatus('error')
      ...
      const body = [
        ...
        transformed.html,
        createElementHTML('script', transformed.code)
      ].join('\n')
        ...
        if (shouldUpdateGist) {
          ...
          if (token) {
            ...
            axios(`https://api.github.com/gists/${data.id}`, {
              ...
              params,
              data: {
                description: `Try it online! https://onizuka-aniki.github.io/codepan/gist/${data.id}`
              }
