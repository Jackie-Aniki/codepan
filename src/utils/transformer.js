...
async function loadBabel() {
  ...
  const [, VuePreset, VueJSXMergeProps] = await Promise.all([
    ...
    ), // use umd bundle since we don't want to parse `require`
    import(
      /* webpackChunkName: "babel-stuffs" */ "!raw-loader!./vue-jsx-merge-props"
    ),
  ]);
...
async function loadPug() {
  ...
  await Promise.all([
    ...
    import(
      /* webpackChunkName: "codemirror-mode-pug" */ "codemirror/mode/pug/pug"
    ),
  ]);
  progress.done();
}
...
async function loadReason() {
  ...
  progress.start();
  await asyncLoad(
    ["/vendor/reason/bs.js", "/vendor/reason/refmt.js"],
    "reason"
  );
  progress.done();
}
...
async function loadCssnext() {
  ...
  progress.start();
  await asyncLoad(
    [process.env.CSSNEXT_CDN, process.env.POSTCSS_CDN],
    "cssnext"
  );
  progress.done();
}
